...
We use an intuitive wrapper for generating sky coordinate grids (but really any grids).
...
This approach is intended for small sky patches, where planar approximations in RA/Dec are valid.
...
We avoid memory-heavy 2D intermediate grids (e.g., with np.meshgrid) and instead construct the final 1D coordinate catalog directly, which is the minimal representation required by SkyCoord, using np.tile and np.repeat tools.
